	...
	function grace_mag_posted_on( $display_meta ) {
        ...
        if( $display_meta == true ) {
            ...
            $posted_on = sprintf(
                ...
                esc_html_x( 'Posted on %s', 'post date', 'grace-mag' ),
                '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><time class="entry-date published">' . $time_string . '</time></a>'
            );

            echo '<span class="posted-date"><em class="meta-icon"><i class="fa fa-clock-o"> </i></em> ' . $posted_on . '</span>'; // WPCS: XSS OK.
